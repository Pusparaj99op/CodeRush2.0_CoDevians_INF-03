...
// ─── Exact React Bits SpecularButton source (TypeScript port) ────────────────
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const h = btn.offsetHeight || btn.getBoundingClientRect().height;
      if (!w || !h) return;
      sizeRef.w = w;
      sizeRef.h = h;
    ...
    const update = (now: number) => {
      ...
      const steer =
        (p.followMouse as boolean) &&
        pointerAngle != null &&
        (!(p.autoAnimate as boolean) || proximityT > 0);
      ...
      program.uniforms.uShineSize.value =
        ((p.shineSize as number) * Math.PI) / 180;
      program.uniforms.uShineFade.value =
        ((p.shineFade as number) * Math.PI) / 180;
