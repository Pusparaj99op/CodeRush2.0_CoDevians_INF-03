...
export function X402InspectorModal({
  ...
  async function runPipeline() {
    setRunning(true);
    // Simulate realistic 400ms cryptographic computation delay
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-4xl overflow-hidden rounded-3xl border border-white/15 bg-[#0d0b09] shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={runPipeline}
              disabled={running}
              className="btn-spectacular flex items-center gap-2 px-4 py-2 text-xs font-semibold"
            >
              {running ? (
                <>
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="grid h-8 w-8 place-items-center rounded-full text-[var(--color-muted)] hover:bg-white/10 hover:text-white"
              >
                <X size={18} />
              </button>
            )}
        ...
        {/* 10-Card Metric Grid */}
        <div className="grid grid-cols-2 gap-3 border-b border-white/10 bg-white/[0.01] p-6 sm:grid-cols-5 text-xs">
          ...
          {activeTab === "http" && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 font-mono text-xs">
              <div className="rounded-2xl border border-white/10 bg-black p-4">
                ...
                <pre className="whitespace-pre-wrap text-amber-300 leading-relaxed overflow-x-auto">
                  {result?.httpRequest ?? ""}
                </pre>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black p-4">
                ...
                <pre className="whitespace-pre-wrap text-emerald-400 leading-relaxed overflow-x-auto">
                  {result?.httpResponse ?? ""}
                </pre>
              </div>
            </div>
